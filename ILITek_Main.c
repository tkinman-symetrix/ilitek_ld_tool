...
int viGetPanelInfor_V3()
{
    ...
    if (GetKernelVer() != _FAIL)
    {
        if (GetFWVersion() != _FAIL)
        {
            if (GetProtocol() != _FAIL)
            {
				if (GetCoreVersion() != _FAIL)
				{
					if(GetFWMode() != _FAIL)
					{
						if (PanelInfor_V3() != _FAIL || GetICMode() != _FAIL)
						{
							ret = _SUCCESS;
						}
					}
		...
		inConnectStyle != _ConnectStyle_I2CHID_)
    {
        ...
        for (count = 1; count < 13; count++)
        {
            if (InitDevice() != _FAIL)
            {
                ...
                if (ICMode != OP_MODE_BOOTLOADER)
                {
                    PRINTF("%s, current op mode is application mode, wait 5 sec, after change to bootloader mode, 0x%X, %u\n", __func__, ICMode, count);
                    usleep(5000000);
                    if (count == 12)
                    {
                        return _FAIL;
                ...
                {
                    PRINTF("%s, check again, current op mode is bootloader mode, can update firmware, 0x%X, ret=%u\n", __func__, ICMode, ret);
                    break;
                }
            }
            else
            {
                PRINTF("%s, current op mode is application mode, wait 5 sec, after change to bootloader mode, %u\n", __func__, count);
                usleep(5000000);
                if (count == 12)
                {
                    return _FAIL;
    ...
    if (inConnectStyle != _ConnectStyle_I2C_)
    {
        ...
        for (count = 1; count < 13; count++)
        {
            if (InitDevice() != _FAIL)
            {
                ...
                {
                    PRINTF("%s, current op mode is bootloader mode, wait 5 sec, after change to application mode, %u\n", __func__, count);
                    usleep(5000000);
	...
	if(strcmp(argv[2], "-h") == 0 || strcmp(argv[2], "--help") == 0 || strcmp(argv[2], "-help") == 0)
	{
		for( u8ID = 0; u8ID < FUNC_NUM; u8ID++ )
		{
			if(strcmp(argv[1], au8FuncStrs[u8ID].FuncStrs) == 0)
			{
				...
				PRINTF("%-16s|%-16s|%-16s|%-16s|%-16s|%-16s|%-16s|%-16s|\n", "Function", "Interface", "Protocol", "Device", "I2C address", "Ctrl param1", "Ctrl param2", "Ctrl param3");
				PRINTF("%-16s|%-16s|%-16s|%-16s|%-16s|%-16s|%-16s|%-16s|\n", "----------------", "----------------", "----------------", "----------------", "----------------", "----------------", "----------------", "----------------");
				for(u8i=0;u8i<INTERFACE_NUM;u8i++)
				{
					PRINTF("%-16s|%-16s|%-16s|%-16s|%-16s|%-16s|%-16s|%-16s|\n", au8FuncStrs[u8ID].FuncStrs, au8FuncStrs[u8ID].Para[u8i].Interface, au8FuncStrs[u8ID].Para[u8i].Protocol, au8FuncStrs[u8ID].Para[u8i].Device, au8FuncStrs[u8ID].Para[u8i].Addr, au8FuncStrs[u8ID].Para[u8i].Ctrl_para1, au8FuncStrs[u8ID].Para[u8i].Ctrl_para2, au8FuncStrs[u8ID].Para[u8i].Ctrl_para3);
					PRINTF("%-16s|%-16s|%-16s|%-16s|%-16s|%-16s|%-16s|%-16s|\n", "----------------", "----------------", "----------------", "----------------", "----------------", "----------------", "----------------", "----------------");	
				}
				ret = _SUCCESS;
				break;
